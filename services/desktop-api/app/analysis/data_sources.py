...
class EastmoneyFundFlowProvider:
    ...
    def fetch(self, code: str) -> FundFlowFacts:
        ...
        clean = code[2:]
        params = {"lmt": 30, "klt": 101, "secid": f"{market}.{clean}", "fields1": "f1,f2,f3,f7", "fields2": "f51,f52,f53,f54,f55,f56,f57,f58,f59,f60,f61,f62,f63,f64,f65"}
        try:
            payload = _json_request("https://push2.eastmoney.com/api/qt/stock/fflow/daykline/get?" + urlencode(params), self.timeout_seconds)
...
class EastmoneyIndustryProvider:
    ...
    def fetch(self, code: str) -> IndustryFacts:
        ...
        clean = code[2:]
        try:
            stock = _json_request(f"https://push2.eastmoney.com/api/qt/stock/get?secid={market}.{clean}&fields=f57,f58,f127", self.timeout_seconds)
            industry = stock.get("data", {}).get("f127")
            if not industry:
                raise ValueError("industry unavailable")
            rows = self._industries()
            matches = [row for row in rows if row.get("f14") == industry]
            if not matches:
                return IndustryFacts(name=industry, status="degraded", error="industry not found in heat table")
            ...
            return IndustryFacts(name=industry, rank=rank, total=len(ordered), change_pct=_number(row.get("f3")),
                                 main_inflow=round(_number(row.get("f62")) / 1e8, 4) if _number(row.get("f62")) is not None else None, status="ok")
        except (HTTPError, URLError, TimeoutError, OSError, ValueError, json.JSONDecodeError) as exc:
            return IndustryFacts(error=str(exc))

    def _industries(self) -> list[dict]:
        now = time.time()
        if self._cache and now - self._cache[0] < 300:
            return self._cache[1]
        params = {"pn": 1, "pz": 600, "fs": "m:90+t:2", "fields": "f12,f14,f3,f62"}
        payload = _json_request("https://push2.eastmoney.com/api/qt/clist/get?" + urlencode(params), self.timeout_seconds)
        rows = list((payload.get("data", {}).get("diff") or {}).values())
