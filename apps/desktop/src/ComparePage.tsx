...
export function ComparePage({onOpenStock}: Props) {
  ...
  return <>
    ...
    {!reports.length ? <section className="panel compare-empty"><BarChart3 /><h2>选择股票后开始比较</h2><p>价格统一以起点 100 标准化，避免直接拿 100 元和 10 元股票比较价格高低。</p></section> : <>
      ...
      <section className="compare-cards">{reports.map((report, index) => <article className="panel compare-card" key={report.stock_code}><div className="compare-card-head"><span style={{background: colors[index]}} /><button type="button" onClick={() => onOpenStock(report.stock_code)}><strong>{report.stock_name}</strong><small>{report.stock_code} · {report.sector}</small></button><b>{report.zhixing_index.toFixed(0)}</b></div><p>{report.diagnosis.summary}</p><div className="compare-card-tags"><span>{report.zhixing_level}</span><span>{report.factor_coverage} 可用</span><span>{report.enrichment_status === 'stale' ? '缓存数据' : report.enrichment_status === 'ok' ? '增强完整' : '增强缺失'}</span></div><div className="compare-evidence"><div><b>优势</b>{report.diagnosis.positive_evidence.slice(0, 2).map(item => <small key={item}>＋ {item}</small>)}</div><div><b>风险</b>{report.diagnosis.risk_evidence.slice(0, 2).map(item => <small key={item}>－ {item}</small>)}</div></div></article>)}</section>
