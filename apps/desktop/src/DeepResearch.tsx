...
function TechnicalGuide() {
  return <details className="technical-guide"><summary>技术指标说明（点击展开）</summary><div className="technical-guide-grid"><article><h3>MACD 金叉 / 死叉</h3><p>MACD 用 DIF 与 DEA 两条趋势线观察动能。DIF 从下向上穿过 DEA 叫“金叉”，表示短期动能改善；从上向下穿过叫“死叉”，表示动能转弱。</p><small>金叉不是单独买入信号，还要结合价格位置、成交量和周线方向。</small></article><article><h3>RSI14</h3><p>RSI 衡量近期上涨和下跌的相对强弱，范围 0～100。通常高于 70 表示短线偏热，低于 30 表示短线偏弱，但强趋势中可以长期维持高位。</p><small>本项目使用 Wilder 平滑 RSI；平盘序列按中性 50 处理。</small></article><article><h3>PE 是什么？多少较好？</h3><p>PE=股价÷每股收益，表示按当前盈利水平需要多少年覆盖估值。PE 越低不一定越好，可能对应盈利下滑、周期高点或公司风险。</p><small>不能用统一数字判断“好坏”，应结合行业、盈利增速和历史分位；亏损股 PE 无效。</small></article><article><h3>日线和周线</h3><p>日线更适合观察近期节奏和入场位置；周线更适合判断中期趋势。日线转强但周线仍下跌，属于短线反弹或周期分歧，不宜直接升级为建仓。</p><small>本项目的操作建议会同时参考日线趋势和数据完整度。</small></article><article><h3>20 日区间下沿 / 上沿</h3><p>下沿是前 20 根 K 线低点附近，上沿是前 20 根 K 线高点附近。当前 K 线只用于判断是否突破或跌破。</p><small>跌破下沿后，价格更低不代表风险更小；需要先观察能否重新站回失效线。</small></article><article><h3>量比与 ATR</h3><p>量比比较当前成交量和近期成交量，ATR 衡量平均波动幅度。盘中成交量尚未走完时，本项目不直接计算全天量比，避免把盘中缩量误判为弱势。</p><small>价格区间使用 ATR 调整，仍需结合交易日和数据状态理解。</small></article></div></details>
}
...
export function DeepResearch({ initialStock = '', onBack }: Props) {
  ...
  async function run() {
    ...
    try { const next = await api.analyze(stock, holding, cost ? Number(cost) : undefined); setReport(next); setSaveMessage(''); setSignalMessage('') }
  ...
  async function addSignalCandidate() {
    if (!report) return
    setError(''); setSignalMessage('正在加入…')
    try {
      await api.addAnalysisSignal(report)
      setSignalMessage('已加入信号核验；同一交易日和同一结论不会重复创建')
    } catch (reason) { setError(String(reason)); setSignalMessage('') }
  }
  ...
  return <>
    ...
    {!report ? <section className="panel deep-empty"><BrainCircuit /><h2>输入一只股票开始分析</h2><p>首版使用腾讯实时行情和前复权日线，缺失的资金、财务、板块和新闻字段会明确标注。</p></section> : <>
      ...
      <section className="supplement-grid">
        <article className="panel source-panel"><div className="panel-title"><h2>资金流</h2><SourceBadge status={report.fund_flow.status} source={report.fund_flow.source} endpoint={report.fund_flow.endpoint} cacheExpired={report.fund_flow.cache_expired} error={report.fund_flow.error} /></div><div className="source-metrics"><div><span>主力净流入（亿元）</span><NumberValue value={report.fund_flow.main_inflow} /></div><div><span>{report.fund_flow.ratio_kind ? '主力占比（估算）' : '主力净流入占比'}</span><NumberValue value={report.fund_flow.main_flow_ratio != null ? report.fund_flow.main_flow_ratio * 100 : undefined} suffix="%" /></div></div><small className="source-note">交易日：{report.fund_flow.trade_date ?? '—'}{report.fund_flow.main_inflow_5d != null ? ` · 腾讯5日 ${report.fund_flow.main_inflow_5d.toFixed(4)}亿` : ''}{report.fund_flow.main_inflow_10d != null ? ` · 10日 ${report.fund_flow.main_inflow_10d.toFixed(4)}亿` : ''}<DataAge seconds={report.fund_flow.data_age_seconds} status={report.fund_flow.status} cacheUsed={report.fund_flow.cache_used} />{report.fund_flow.error ? ` · ${report.fund_flow.error}` : ''}</small></article>
        <article className="panel source-panel"><div className="panel-title"><h2>财务增速</h2><SourceBadge status={report.finance.status} source={report.finance.source} cacheExpired={report.finance.cache_expired} error={report.finance.error} /></div><div className="source-metrics"><div><span>营收同比</span><NumberValue value={report.finance.revenue_yoy} suffix="%" /></div><div><span>净利润同比</span><NumberValue value={report.finance.profit_yoy} suffix="%" /></div><div><span>ROE</span><NumberValue value={report.finance.roe} suffix="%" /></div></div><small className="source-note">报告期：{report.finance.report_date ?? '—'} · 公告日：{report.finance.notice_date ?? '—'}<DataAge seconds={report.finance.data_age_seconds} status={report.finance.status} cacheUsed={report.finance.cache_used} />{report.finance.error ? ` · ${report.finance.error}` : ''}</small></article>
      ...
      <div className="deep-grid"><section className="panel"><div className="panel-title"><h2>行情与日周线</h2><span className="pill">日线 {report.technical.trend} · 周线 {report.weekly.trend}</span></div><div className="metric-grid">{[['现价', report.quote.price], ['涨跌', report.quote.change_pct != null ? `${report.quote.change_pct.toFixed(2)}%` : null], ['PE', report.quote.pe], ['MA5', report.technical.ma5], ['MA20', report.technical.ma20], ['MA60', report.technical.ma60], ['周MA5', report.weekly.ma5], ['RSI14', report.technical.rsi14], ['量比', report.technical.volume_ratio], ['前20日下沿', report.technical.prior_support20], ['前20日上沿', report.technical.prior_resistance20]].map(([label, value]) => <div key={label as string}><span>{label}</span><strong>{typeof value === 'number' ? value.toFixed(2) : value ?? '—'}</strong></div>)}</div>{report.technical.macd && <p className="signal-line">MACD：{report.technical.macd.golden_cross ? '金叉' : report.technical.macd.death_cross ? '死叉' : report.technical.macd.hist > 0 ? '多头' : '空头'} · DIF {report.technical.macd.dif.toFixed(3)} · DEA {report.technical.macd.dea.toFixed(3)}</p>}</section>
      ...
      <section className="panel advice-panel"><div className="panel-title"><div><span className="step">03</span><h2>操作参考与价格区间</h2></div><span className="pill">仅研究参考 · {report.advice.review_after}复核</span></div><div className="advice-head"><div className="advice-action">{report.advice.action}</div><div><strong>{report.advice.category}</strong><p>{report.advice.summary}</p></div><span className="status">风险：{report.advice.risk_level} · 数据完整度 {report.advice.data_completeness.toFixed(0)}%</span></div><div className="evidence-grid advice-conditions"><div><h3>已触发</h3>{report.advice.triggered_conditions.map(item => <p className="evidence-positive" key={item}>＋ {item}</p>)}</div><div><h3>尚未满足</h3>{report.advice.unmet_conditions.map(item => <p className="evidence-conflict" key={item}>△ {item}</p>)}</div><div><h3>失效条件</h3>{report.advice.invalidation_conditions.map(item => <p className="evidence-risk" key={item}>－ {item}</p>)}</div></div><div className="zone-grid">{report.advice.zones.map(zone => <article className={`zone-${zone.tone}`} key={zone.name}><strong>{zone.name}</strong><span>¥{zone.low.toFixed(2)} ~ ¥{zone.high.toFixed(2)}</span><small>{zone.action}</small></article>)}</div><p className="deep-note">核心缺失：{report.core_missing_fields.length ? report.core_missing_fields.join('、') : '无'}；增强缺失：{report.enrichment_missing_fields.length ? report.enrichment_missing_fields.join('、') : '无'}。知行指数与操作建议使用同一评分体系；规则指纹：{report.rule_fingerprint}。</p></section>
    </>}
  </>
}
