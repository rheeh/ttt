...
function SourceBadge({status, source}: {status: string; source: string}) {
  return <span className={`source-badge ${status}`}>{status === 'ok' ? '已接入' : status === 'stale' ? '使用缓存' : status === 'degraded' ? '部分数据' : '未接入'} · {source}</span>
}
...
function FreshnessItem({label, info, fallback}: {label: string; info?: FreshnessInfo; fallback?: string}) {
  ...
  const detail = info?.latest_trade_date ? `截至 ${info.latest_trade_date} · ${info.bar_count ?? 0} 条` : info?.report_date ? `报告期 ${info.report_date}` : info?.trade_date ? `交易日 ${info.trade_date}` : info?.age_seconds != null ? `更新于 ${freshnessAge(info.age_seconds)}` : fallback ?? '时间未知'
